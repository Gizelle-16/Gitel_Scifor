...
def app_mode_recc():
    ...
    class Display:
        ...
        def display_bmi(self,person):
            ...
            st.markdown(
                """
                A Healthy BMI range is between 18.5 kg/m²to 25 kg/m².
                """)   
        ...
        def display_recommendation(self,person,recommendations):
            st.header('DIET RECOMMENDOR')  
            with st.spinner('Generating recommendations...'): 
                meals=person.meals_calories_perc
                st.subheader('Recommended recipes:')
                for meal_name,column,recommendations in zip(meals,st.columns(len(meals)),recommendations):
                    with column:
                        ...
                        for recipe in recommendations:                   
                            if isinstance(recipe, dict):
                                ...
                                # Create a DataFrame for nutritional values
                                nutritions_df = pd.DataFrame({value: [recipe.get(value, 0)] for value in nutritions_values})
                                
                                # Display nutritional values in the expander
                                ...
                                # Display ingredients in the expander
                                expander.markdown(f'<h5 style="text-align: center;font-family:sans-serif;">Ingredients:</h5>', unsafe_allow_html=True)
                                
                                if 'RecipeIngredientParts' in recipe and isinstance(recipe['RecipeIngredientParts'], list):
                                    for ingredient in recipe['RecipeIngredientParts']:
                                        expander.markdown(f"- {ingredient}")
                                else:
                                    expander.markdown("- No ingredients available")
                                
                                # Display recipe instructions in the expander
                                expander.markdown(f'<h5 style="text-align: center;font-family:sans-serif;">Recipe Instructions:</h5>', unsafe_allow_html=True)
                                
                                if 'RecipeInstructions' in recipe and isinstance(recipe['RecipeInstructions'], list):
                                    for instruction in recipe['RecipeInstructions']:
                                        expander.markdown(f"- {instruction}")
                                else:
                                    expander.markdown("- No instructions available")
                                
                                # Display cooking and preparation time in the expander
                                ...
                                expander.markdown(f"""
                                        - Cook Time       : {recipe.get('CookTime', 'Unknown')}min
                                        - Preparation Time: {recipe.get('PrepTime', 'Unknown')}min
                                        - Total Time      : {recipe.get('TotalTime', 'Unknown')}min
                                    """)
                            else:
                                st.warning(f"Unexpected format for recipe: {recipe}")
    ...
    with st.form("recommendation_form"):
        ...
        activity = st.selectbox('HOw active are you: ',options=['Little/no exercise', 'Light exercise', 'Moderate exercise (3-5 days/wk)', 'Very active (6-7 days/wk)', 
